...
def log_lead_to_crm(lead: LeadInfo, transcript_summary: str = ""):
    """Send the extracted lead data + transcript silently to Propello CRM Webhooks."""
    if not lead.phone:
        return # CRM requires a phone number to match/create leads
    
    budget_str = str(lead.budget or "").lower()
    budget_val = None
    try:
        import re
        b = re.sub(r'[^\d.]', '', budget_str)
        if b:
            budget_val = float(b)
            if "cr" in budget_str: budget_val *= 10000000
            elif "l" in budget_str or "lac" in budget_str: budget_val *= 100000
    except: pass
    ...
    payload = {
        ...
        "location_preference": lead.location,
        "personal_notes": f"Appointment/Timeline preference: {lead.appointment}" if lead.appointment else None,
        "transcript_summary": transcript_summary
    }

    crm_url = os.environ.get("CRM_WEBHOOK_URL", "http://localhost:8000/api/webhooks/priya")
    secret = os.environ.get("PRIYA_WEBHOOK_SECRET", "supersecret-priya-key-change-me")
    ...
    def fire():
        try:
            req = urllib.request.Request(
                crm_url,
                data=json.dumps(payload).encode("utf-8"),
                headers={"Content-Type": "application/json", "X-Priya-Secret": secret}
            )
...
def build_system_prompt(
    ...
    known = []
    if memory_brief:
        known.append(f"PAST CONVERSATION MEMORY: {memory_brief}")
        known.append("IMPORTANT: Do NOT ask them for their name, budget, or requirements again if they are already in the memory profile! Acknowledge their past history warmly.")
...
Whenever you detect the user sharing any of these details, silently note them by appending these exact tags exactly ONCE at the VERY END of your absolutely final response paragraph, on a new line:
[LEAD:name=VALUE], [LEAD:phone=VALUE], [LEAD:budget=VALUE], [LEAD:location=VALUE], [LEAD:appointment=Date and Time]
CRITICAL RULE: These tracking tags MUST remain strictly in English syntax. Do NOT translate the tag keys into other languages (e.g., use "[LEAD:budget=...]", NOT "[LEAD:बजट=...]").
...
def parse_lead_tags(raw: str, existing: LeadInfo) -> tuple[str, LeadInfo]:
    ...
    updated = existing.model_dump()
    for key in ("name", "phone", "budget", "location", "appointment"):
        # Immune to spacing: [ LEAD : phone = 123 ] 
        match = re.search(rf"\[\s*LEAD\s*:\s*{key}\s*=\s*([^\]]+)\]", raw, flags=re.IGNORECASE)
        if match and not updated.get(key):
            updated[key] = match[1].strip()
            
    # Aggressively strip anything resembling a lead tag
    clean = re.sub(r"\[\s*LEAD\s*:.*?\]", "", raw, flags=re.IGNORECASE).strip()
    # Let's also strip orphaned brackets if model messes up badly
    clean = re.sub(r"\[\s*LEAD.*?\]", "", clean, flags=re.IGNORECASE).strip()
    return clean, LeadInfo(**updated)
...
def extract_lead_from_user_message(message: str, existing: LeadInfo) -> LeadInfo:
    ...
    if not updated.get("budget"):
        budget_match = re.search(
            r"\b(?:under|around|about|upto|up to|budget\s*(?:is|around|about)?\s*)?"
            r"((?:₹|rs\.?|inr)?\s*(?:\d+(?:[\.,]\d+)?|dedh|dhai|do|ek|teen|chaar|paanch|sadhe|pachas|saath|assi|nabbe|sau)\s*(?:cr|crore|lakh|lac|million|k))\b",
            text,
            flags=re.IGNORECASE,
        )
        if budget_match:
...
def prepare_chat_context(body: ChatRequest) -> tuple[list[dict], str, LeadInfo, Optional[dict]]:
    ...
    memory_brief = None
    if merged_lead.phone:
        try:
            import urllib.request
            import json
            url = f"http://localhost:8000/api/contacts/lookup/{merged_lead.phone}"
            req = urllib.request.Request(url, headers={"Content-Type": "application/json"})
            with urllib.request.urlopen(req, timeout=2) as resp:
                data = json.loads(resp.read().decode("utf-8"))
                if data and data.get("priya_memory_brief"):
                    memory_brief = data.get("priya_memory_brief")
        except:
            pass
        ...
        try:
            ...
            for chunk in stream:
                ...
                while True:
                    ...
                    sentence = match.group(1).strip()
                    buffer = buffer[match.end():]
                    sentence_clean = re.sub(r"\[\s*LEAD.*?\]", "", sentence, flags=re.IGNORECASE).strip()
                    if sentence_clean:
                        payload = {"type": "chunk", "text": sentence_clean}
                        yield f"data: {json.dumps(payload, ensure_ascii=False)}\n\n"
            ...
            trailing = re.sub(r"\[\s*LEAD.*?\]", "", buffer, flags=re.IGNORECASE).strip()
            if trailing:
                payload = {"type": "chunk", "text": trailing}
                yield f"data: {json.dumps(payload, ensure_ascii=False)}\n\n"
            ...
            raw = "".join(all_text_parts).strip()
            _, new_lead = parse_lead_tags(raw, merged_lead)
            new_lead = extract_lead_from_user_message(body.message, new_lead)
            log_lead_to_csv(new_lead)
            log_lead_to_crm(new_lead, raw)
